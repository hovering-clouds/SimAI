...
全局参数 `BW_CHANGE_THRESHOLD_PCT`（`analytical.py`），默认 0.0。
当 `old_bw > 0` 且带宽变化幅度 ≤ 阈值时不推送新事件。

**实测效果（2P2D MFS，阈值 1%）：**

| 指标     | 优化前    | 优化后    | 改善             |
| -------- | --------- | --------- | ---------------- |
| 总事件数 | 7,831,609 | 3,889,337 | **-50.3%** |
| 队列峰值 | 1,018,303 | 30,620    | **-97.0%** |
| Makespan | 5,232 ms  | 5,241 ms  | +0.17%           |

CPU 时间节省有限（~5%），因主要开销是遍历活跃流和调用 allocator，而非事件推送。

### 事件批处理（方案二的简化版已落地）

**对应问题：** 同一时间戳有大量 events 排队，每个都触发一次 `_reallocate_bandwidth`。
且 `_handle_compute_done` 中步骤 4 realloc 与 drain 内部 realloc 存在冗余。

**实现方式：** 事件循环中，对每个时间戳窗口先处理完所有 events
（含 handler 新推入的零字节流事件），最后做一次 realloc。
通过 `_skip_reallocate` 标志推迟内部 realloc。全局参数 `EVENT_BATCH_GAP_US` 控制批处理窗口。

**实测效果（2P2D MFS，无阈值过滤，gap=0）：**

| 指标             | 优化前    | 优化后           | 改善                   |
| ---------------- | --------- | ---------------- | ---------------------- |
| realloc 调用次数 | 2,430,609 | **78,611** | **-96.8%**       |
| 总运行时间       | ~18 min   | **~2 min** | **-88%**         |
| Makespan         | 5,232 ms  | 5,240 ms         | +0.15%（DES 顺序差异） |

关于 0.15% 的 makespan 差异：这是 DES 中同一时间戳多事件处理顺序的固有不确定性。
batch 模式将所有事件处理完再 realloc，会将已在同一时间戳完成并移除的零字节流排除在 realloc 外；
而逐个处理模式中，另一条流的 handler 可能先启动共享流、在 75461 仍存活时腰斩其带宽。
两种结果在 DES 中均有效。

个人备注：主要原因在于batch处理会一次性吃掉好几个event，与单独处理每个event的区别在于，有的时候某个event的结束会触发新流的加入，可能导致原本在当前时间戳结束的流，因为分配带宽的变化而变慢，flow version++，原本已经进batch的事件被invalidate，因此batch处理和单独处理有些许差异。本质上这是因为同一个时间戳的不同事件的执行顺序未定义导致的。事实上同一时间戳的两个event理论上不应该能够有一个影响另一个的情况。

### min_bw_epsilon 阈值

修复 allocator 中 `rem <= 0` 浮点 epsilon 残留，避免误分配极低带宽引发时间戳溢出。
全局参数 `MIN_BW_REMAINING = 1e-6`（`base_allocator.py`）。
