...
**包含**：
- `RoutingHints` 数据类：路径缓存 + 链路负载统计
- `compute_routing_hints(topology, workload)` 函数：遍历所有 flow task，计算最短路径并聚合 link_loads
- `_bfs_shortest_path(topology, src, dst)`：BFS 最短路径算法（按跳数）
- `get_flow_links(task, topology)`：将 flow 的缓存路径转为物理链路列表
- `get_most_used_links(top_k)`：按负载排序返回热点链路

**不包含**：
- ECMP 多路径支持（当前仅返回单条最短路径）
- 动态拓扑感知（假设拓扑是静态的）
- 带权最短路径（BFS 按跳数，不考虑带宽权重）
...
def compute_routing_hints(topology: NetworkTopology, workload: P2PWorkload) -> RoutingHints
```
...
```
tests/test_routing_hints.py: 34 passed
tests/test_topology_loader.py: 52 passed
tests/ (完整回归): 239 passed (153 原有 + 52 topology + 34 routing)
```
...
*开发时间：2026-04-16（初始实现 + 双向拓扑修正）*
*测试状态：34 passed（routing hints）+ 52 passed（topology loader）+ 153 passed（原有）= 239 total*
*重要修正：*
