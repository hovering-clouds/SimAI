"""
1F1B end-to-end simulation: AICB workload → P2PWorkload → OneFOneBAnalyzer → AnalyticalExecutor

Usage:
    uv run python scripts/run_e2e_1f1b.py
    uv run python scripts/run_e2e_1f1b.py --aicb <path> --topo <path>
"""
...
def main():
    ...
    aicb_file = args.aicb
    topo_file = args.topo
    output_dir = args.output or "outputs/e2e_1f1b"
    ...
    # ============================================================
    # Step 3: Load topology & run 1F1B static analysis
    # ============================================================
    print()
    print("=" * 60)
    ...
    analysis = OneFOneBAnalyzer(topology).analyze(workload)
    print(f"  Nodes with compute tasks: {len(analysis.execution_plan.compute_order)}")
    print(f"  Schedule: 1F1B (pp={pp}, ga={header.ga})")
